...
typealias DurationFormatter = Formatter<Duration>
...
private fun buildFormatter(locale: Locale, units: Map<String, DurationUnitNames>): DurationFormatter = { duration ->
...
private fun buildDurationFormatter(tag: String, locale: Locale): DurationFormatter? {
    return when (tag) {
        "en", "af", "nl", "id", "ms", "sw" ->
            buildFormatter(locale, u2("second","minute","hour","day","week","month","year",
        ...
        "cs" -> buildFormatter(locale, u3("sekunda","minuta","hodina","den","týden","měsíc","rok",
                                   ...
                                   "sekund","minut","hodin","dní","týdnů","měsíců","let"))
        ...
        "sl" -> buildFormatter(locale, u3("sekunda","minuta","ura","dan","teden","mesec","leto",
                                   ...
                                   "секунде","minute","ure","dni","tednov","mesecev","let"))
...
fun durationFormatterFor(locale: Locale): DurationFormatter {
    ...
    while (currentTag.isNotEmpty()) {
        val cached = durationCache[currentTag]
        if (cached != null) return cached
        val built = buildDurationFormatter(currentTag, locale)
        if (built != null) {
            durationCache[currentTag] = built
            return built
        }
        currentTag = currentTag.substringBeforeLast('-', "")
    }
    return durationCache.getOrPut("en") { buildDurationFormatter("en", Locale("en"))!! }
}
